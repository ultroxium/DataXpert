...
const addTeamMember = async (data: any) => {
  const response = await axios.post('/utils/api?type=invite', data);
  return response.data;
};

const deleteMember = async (id: string) => {
  const response = await axios.delete(`/utils/api?id=${id}`);
  return response.data;
};

const updateRoles = async (id: string, data: any) => {
  const response = await axios.put(`/utils/api?id=${id}&type=roles`, data);
  return response.data;
};

export function LinkShare({ data }) {
                ...
                <div className="flex items-center space-x-4 xxs:w-full md:w-fit">
                  ...
                  <div>
                    <p className="text-14 font-medium leading-none flex gap-2">
                      ...
                      {member?.user_obj ? (
                        <span
                          className={`px-2 py-1 rounded-full text-10 ${
